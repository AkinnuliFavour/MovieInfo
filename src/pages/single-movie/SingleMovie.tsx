...
import { ReactComponent as Bookmark1 } from "../../assets/Icons/Bookmark-1.svg";
...
const SingleMovie = () => {
  ...
  const trailer = movieVideos
    ? movieVideos?.data.results.find((result) => result.type === "Trailer")
    : "";
  const trailerKey = trailer ? trailer.key : "";

  const stars = movieCredits
    ? movieCredits?.data.cast.filter((actor) => actor.popularity > 60)
    : "";
  const starNames = stars ? stars.map((star) => star.name) : null;
  const actorNames = movieCredits
    ? movieCredits?.data.cast.map((actor) => actor.name)
    : "";
  ...
  return (
    <main className="flex flex-col lg:flex-row h-screen max-w-screen">
      <Nav hidden={true} />
      <SideNav />
      <section className="flex flex-col px-4 lg:ml-[263px] lg:mr-6 mt-[38px] w-full h-full lg:w-5/6 2xl:ml-[20%]">
        <iframe
          width="420"
          height="449"
          src={`https://www.youtube.com/embed/${trailerKey}`}
          className="w-full min-h-[449px] bg-no-repeat bg-contain bg-center rounded-[30px]"
        ></iframe>
        <div className="flex flex-col lg:flex-row gap-4 px-[18px] mt-[31px]">
          <span
            data-testid="movie-title"
            className="text-[#404040] font-medium text-[23px]"
          >
            {movieDetails?.data.original_title} • {date.getFullYear()} • {Pg} •{" "}
            {`${hours}h ${minutes}m`}
          </span>
        ...
        <p className="mt-4 px-[18px] font-normal" data-testid="movie-overview">
          {movieDetails?.data.overview}
        </p>
        <p className="mt-4 px-[18px]" data-testid="movie-release-date">
          Director: <span className="text-[#BE123C]">{director?.name}</span>
        </p>
        <p className="mt-4 px-[18px]" data-testid="movie-runtime">
          Stars:{" "}
          <span className="text-[#BE123C]">
            {starNames?.join(", ") || mainActors?.join(", ")}
          </span>
        </p>
        <div className="lg:flex justify-between">
          <p className="mt-4 px-[18px]">
            Rating:{" "}
            <span className="text-[#BE123C]">
