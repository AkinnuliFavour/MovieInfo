    ...
    return (
        <main className="min-h-screen bg-background text-white font-sans selection:bg-primary/30">
            ...
            <div className="container mx-auto px-6 pt-24 pb-12">
                ...
                <div className="flex justify-between items-center mb-8 bg-white/5 p-4 rounded-2xl backdrop-blur-md border border-white/10 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
                    <button onClick={prevMonth} className="p-2 hover:bg-white/10 rounded-full transition-colors text-primary">
                        <ChevronLeft className="h-6 w-6" />
                    </button>
                    <h2 className="text-xl font-bold text-white">
                        {currentMonth.toLocaleString('default', { month: 'long', year: 'numeric' })}
                    </h2>
                    <button onClick={nextMonth} className="p-2 hover:bg-white/10 rounded-full transition-colors text-primary">
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-7 gap-2 md:gap-4 animate-fade-in-up" style={{ animationDelay: '200ms' }}>
                        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                            <div key={day} className="text-center font-medium text-gray-400 py-2 text-sm uppercase tracking-wider">{day}</div>
                        ))}
                        
                        {Array.from({ length: firstDayOfMonth }).map((_, i) => (
                            <div key={`empty-${i}`} className="h-24 md:h-32 bg-white/5 rounded-xl opacity-30 border border-white/5"></div>
                        ))}
                            ...
                            return (
                                <div 
                                    ...
                                    className={`min-h-[96px] md:min-h-[128px] bg-white/5 rounded-xl p-2 border transition-all hover:bg-white/10 group relative overflow-hidden ${
                                        isToday ? 'border-primary/50 shadow-glow-sm' : 'border-white/10 hover:border-white/20'
                                    }`}
                                >
                                    <span className={`absolute top-2 right-2 text-sm font-bold ${isToday ? 'text-primary' : 'text-gray-500 group-hover:text-gray-300'}`}>
                                        {day}
                                    </span>
                            ...
                            .map((movie, idx) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        <h3 className="font-bold text-lg text-white group-hover:text-primary transition-colors">{movie.title}</h3>
                                        <p className="text-gray-400 flex items-center gap-2">
                                            <CalendarIcon className="h-4 w-4" />
                                            {new Date(movie.release_date).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                                        </p>
